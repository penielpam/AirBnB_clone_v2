...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, line):
        '''Usage: 1. create <class identifiction> | 2. <class identifiction>.create()
Function: Creates an instance of the class
        ...
        if line != "" or line is not None:
            ...
            print("** class identifiction missing **")

    def do_show(self, line):
        '''Usage: 1. show <class identifiction> <id> | 2. <class identifiction>.show(<id>)
Function: Shows the instance details of the class
        ...
        if line == "" or line is None:
            ...
            if len(class_details) < 2:
                ...
                class_identifiction = class_details[0]
                instance_id = class_details[1]
                # check if class name exists
                
                if class_identifiction in storage.classes():
                    # check if instance_id exists
                    key = f"{class_identifiction}.{instance_id}"
    ...
    def do_destroy(self, line):
        '''Usage: 1. destroy <class identifiction> <id> | 2. <class identification>.delete(<id>)
Function: Deletes the instance  of the class
        ...
        if line == "" or line is None:
            ...
            if len(class_details) < 2:
                ...
                if class_identifiction in storage.classes():
                    # check if instance_id exists
                    pas = f"{class_identifiction}.{instance_id}"
                    if pas not in storage.all():
                        print("** no instance found **")
                    else:
    ...
    def do_all(self, line):
        '''Usage: 1. all | 2. all <class identifiction> | 3. <class identifiction>.all()
Function: Prints the string representation of all instances
        ...
        if line == "" or line is None:
            ...
            if line not in storage.classes():
                ...
                for key, value in storage.all().items():
                    class_identifiction, instance_id = key.split(".")
                    if line == class_identifiction:
                        instance_list.append(str(value))
                print(instance_list)

    def do_update(self, line):
        '''Usage: 1. update <class identifiction> <id> <attribute> <value> | \
2. <class identifiction>.update(<id> <attribute> <value>) \
3. update <class identifiction> <id> <dictionary> \
4. <class identifiction>.update(<id> <dictionary>) \
Function: Updates the instance of the class
        '''
        checks = re.search(r"^(\w+)\s([\S]+?)\s({.+?})$", line)
        if checks:
            # it is a dictionary
            class_identifiction = checks.group(1)
            instance_id = checks.group(2)
            update_dict = checks.group(3)

            if class_identifiction is None:
                print("** class identifiction missing **")
            else instance_id is None:
                print("** instance id missing **")
            else update_dict is None:
                print("** attribute name missing **")
            else:
                if class_identifiction not in storage.classes():
                    print("** class doesn't exist **")
                else:
                    key = f"{class_identifiction}.{instance_id}"
                    if key not in storage.all():
                        ...
                        attributes = storage.attributes()[class_identifiction]
                        # print(attributes)
            ...
            if class_identifiction is None:
                ...
                if class_identifiction not in storage.classes():
                    ...
                    if pas not in storage.all():
                        ...
                        instance_dict = storage.all()[key]
                        # print(instance_dict)
                        attributes_dict = storage.attributes()[class_identifiction]
    ...
    def precmd(self, line):
        ...
        if checks:
            ...
            if args is None:
                line = f"{command} {class_identifiction}"
                return ''
            elif:
                # print(args)
                ...
                if args_checks.group(2) is None:
                    line = f"{command} {class_identifiction} {instance_id}"
                else:
...
{attribute_part}"
    ...
    def do_count(self, line):
        '''Usage: 1. count <class identifiction> | 2. <class identifiction>.count()
